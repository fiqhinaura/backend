...
export const accAntrian = async (req, res) => {
    ...
    antrian.status = 'dalam antrian'; // PERUBAHAN STATUS KE 'dalam antrian'
...
export const getAntrianUser = async (req, res) => {
    try {
        console.log('[BACKEND - getAntrianUser] Fungsi dipanggil.');
        console.log('[BACKEND - getAntrianUser] userId dari req.userId:', req.userId);

        const antrian = await Antrian.findOne({
            where: { userId: req.userId, status: 'dalam antrian' } // Kriteria pencarian
        });

        // --- LOGGING TAMBAHAN UNTUK DEBUGGING HASIL QUERY ---
        if (antrian) {
            console.log('[BACKEND - getAntrianUser] Antrian ditemukan:', antrian.toJSON());
        } else {
            console.log('[BACKEND - getAntrianUser] Antrian tidak ditemukan untuk userId:', req.userId, 'dengan status "dalam antrian".');
            // Opsional: Untuk debugging lebih lanjut, Anda bisa mencari semua antrian user
            const allUserAntrian = await Antrian.findAll({ where: { userId: req.userId } });
            console.log('[BACKEND - getAntrianUser] Semua antrian user (untuk debugging):', allUserAntrian.map(a => a.toJSON()));
        }
